...
\noindent The \textbf{event} \index{Event} is a subset of the sample space. We can think of it as a set consisting of possible outcomes of the experiment. Using the previous example, a possible event of the experiment is $\{ H \}$ where the coin lands head. 
...
\noindent Let us start with defining what conditional probability is. \index{Conditional!Probability}Given two events $A$ and $B$, the \textbf{conditional probability}, denoted by $P(A|B)$, is the probability of event $A$ happening when we know event $B$ has already happened.
